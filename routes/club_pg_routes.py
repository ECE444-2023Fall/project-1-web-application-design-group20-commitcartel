...
from database import insert_one, get_data
...
def clubs(club_id):
    ...
    club_find = get_data('Clubs',{'_id': club_id})
    if(len(club_find)> 0):
        ...
        for eventID in events:
            event_find = get_data('Events',{'_id': eventID})
...
def create_club():
    ...
    if form.validate_on_submit():
        ...
        club_id = insert_one('Clubs',{'club_description': session.get('club_description'), 'club_name': session.get('club_name'), 'email': session.get('email'),'password': session.get('password'), 'club_rating_avg': '', 'events': '', 'followers': '', 'photo':  '' })
